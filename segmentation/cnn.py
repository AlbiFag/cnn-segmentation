...
class CEL_UNet_16(object):
    ...
    def create_model(self):
        ...
        # Input layer
        temp_layer = layers.Input(shape=self.input_size, dtype='float16')
        input_tensor = temp_layer
        ...
        for i in range(self.depth):
            for j in range(2):
                ...
                temp_layer = conv_layer(self.n_initial_filters * pow(2, i), kernel_size=self.kernel_size,
                                        ...
                                        activation='linear',
                                        kernel_regularizer=self.kernel_regularizer,
                                        bias_regularizer=self.bias_regularizer, dtype='float16')(temp_layer)
                # batch normalization
                if self.add_batch_normalization:
                    temp_layer = layers.BatchNormalization(axis=-1, dtype='float16')(temp_layer)
                # activation
                temp_layer = layers.Activation(self.activation, dtype='float16')(temp_layer)
            downsampling_layers.append(temp_layer)
            temp_layer = max_pool_layer(pool_size=self.pool_size,
                                        strides=self.pool_strides,
                                        padding=self.padding, dtype='float16')(temp_layer)
        for j in range(2):
            # Bottleneck
            temp_layer = conv_layer(self.n_initial_filters * pow(2, self.depth), kernel_size=self.kernel_size,
                                    strides=self.strides,
        ...
        for i in range(self.depth):
            ...
            temp_layer_edge = layers.Activation(self.activation, dtype='float16')(temp_layer_edge)
            ...
            temp_layer_mask = conv_transpose_layer(self.n_initial_filters * pow(2, (self.depth - 1) - i),
                                                   ...
                                                   padding=self.padding,
                                                   kernel_regularizer=self.kernel_regularizer,
                                                   bias_regularizer=self.bias_regularizer, dtype='float16')(temp_layer_merge)
            temp_layer_mask = layers.Activation(self.activation, dtype='float16')(temp_layer_mask)
            ...
            for j in range(2):
                temp_layer_edge = conv_layer(self.n_initial_filters * pow(2, (self.depth - 1) - i),
                                             ...
                                             bias_regularizer=self.bias_regularizer, dtype='float16')(temp_layer_edge)
                if self.add_batch_normalization:
                    temp_layer_edge = layers.BatchNormalization(axis=-1, dtype='float16')(temp_layer_edge)
                temp_layer_edge = layers.Activation(self.activation, dtype='float16')(temp_layer_edge)
                ...
                temp_layer_mask = conv_layer(self.n_initial_filters * pow(2, (self.depth - 1) - i),
                                             ...
                                             activation='linear',
                                             kernel_regularizer=self.kernel_regularizer,
                                             bias_regularizer=self.bias_regularizer, dtype='float16')(temp_layer_mask)
                if self.add_batch_normalization:
                    temp_layer_mask = layers.BatchNormalization(axis=-1, dtype='float16')(temp_layer_mask)
                temp_layer_mask = layers.Activation(self.activation, dtype='float16')(temp_layer_mask)
